  ...
  return (
    <main className="w-full justify-between md:flex max-w-[1200px] md:mx-auto">
      ...
      <div className="p-5 md:p-10 md:w-[55%]">
        ...
        <h1 className="text-black my-4 font-bold text-3xl sm:text-4xl lg:text-6xl xl:my-7">{product.title}</h1>

        <p className="text-gray-700 sm:text-lg md:text-xl">{product.description}</p>

        <div className="my-4 flex justify-between items-center md:my-6 lg:flex-col md:items-start">

          <h1 className="text-black text-2xl sm:text-3xl font-bold">${product.price}<span className="text-lg bg-lightorange text-orange font-extrabold px-1 rounded ml-3">{product.discountPercentage}%</span></h1>
